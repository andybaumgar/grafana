...
describe('Variables - Rollup', () => {
  it('can add a new rollup variable', () => {
    e2e.flows.login('admin', 'admin');
    e2e.flows.openDashboard({ uid: `${PAGE_UNDER_TEST}?orgId=1&editview=templating` });

    // Create a new "Rollup" variable
    e2e.components.CallToActionCard.buttonV2('Add variable').click();

    e2e.pages.Dashboard.Settings.Variables.Edit.General.generalTypeSelectV2().within(() => {
      e2e().get('input').type('Rollup{enter}');
    });
    ...
    e2e.pages.Dashboard.Settings.Variables.Edit.RollupVariable.rollupOptionsQueryInputV2().type('pesto').blur();
